...
router.get("/", async (request, response) => {
  try {
    ...
    if (maxPrice) {
      const resultForMaxPrice = await knex("meal").where(
        "price",
        "=<",
        maxPrice
      );
      ...
      const resultForCreatedAfter = await knex("meal").where(
        "created_date",
        "<",
        createdAfter
      );
      ...
      const resultlLimit = await knex("meal").where(
        "max_reservations",
        "=",
        limit
      );
...
router.get("/:mealId", async (request, response) => {
  ...
  try {
    const returnsMealById = await knex("meal")
      .where({ id: mealId })
      .select("title", "id");
...
router.put("/:mealId", async (request, response) => {
  ...
  try {
    const updatesTheMealById = await knex("meal")
      .where({ id: mealId })
      .update(rId);
...
router.delete("/:mealId", async (request, response) => {
  ...
  const rId = request.body;
  try {
    const deletesTheMealById = await knex("meal")
      .where({ id: mealId })
      .del(rId);
